    ...
    <div data-landing className="flex flex-col bg-background text-foreground">
      ...
      <header className="relative z-10 flex items-center justify-between px-8 py-6">
        <Link href="/" className="flex items-center gap-2.5 transition-opacity hover:opacity-75">
          ...
          <span className="font-[sans-serif] text-xl font-bold tracking-tight">Calrix</span>
        </Link>
        <Link href="/signin" className="rounded-full bg-foreground px-5 py-2 font-mono text-sm text-background transition-all hover:-translate-y-px hover:bg-accent">
          Sign in
        </Link>
      </header>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 pb-20 pt-12 text-center">
          ...
          {PLANS.map((plan) => (
            <div
              ...
              className={`flex flex-col rounded-3xl bg-white p-7 text-left shadow-[0_12px_24px_-10px_rgba(255,64,0,0.15)] ${
                plan.featured ? "ring-2 ring-[#ff4000]" : "ring-1 ring-black/5"
              }`}
            >
              ...
              <div className="mt-6 min-h-[72px]">
                <div className="flex items-baseline gap-2">
                  ...
                  {plan.priceNote && (
                    <span className="font-mono text-sm text-muted-foreground">{plan.priceNote}</span>
                  )}
                </div>
                {plan.highlight && (
                  <p className="mt-2 font-mono text-sm text-[#ff4000]">{plan.highlight}</p>
                )}
              </div>
              ...
              <ul className="mt-7 divide-y divide-border border-t border-border">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 py-3.5 text-sm text-foreground/80">
        ...
        <p className="mt-10 text-sm text-muted-foreground">
          Questions?{" "}
          <a href="mailto:hello@calrix.ai" className="underline underline-offset-2 hover:text-foreground">
            Talk to us
          </a>
          .
        </p>
      </div>
      </div>{/* end grid-scoped content */}
      <FooterSection />
    </div>
  );
}
